...
QA 트랙(앵커 보강 + 외부 리뷰 P0 포함) 다시 닫혔다. 남은 건 외부 리뷰의 P1 항목(팀장 관찰
주기, 슬라이더 "관찰 못 함", 주차 평균 추이의 통계적 타당성, 타임라인 6개월 게이팅,
Charter-코드 범위 재정리)과, `CHARTER.md` 리스크1의 마지막 항목인 **Vercel 프로덕션 배포 +
20명 규모 실부하테스트**(일반 처리량 검증, 레이스 컨디션과는 별개 — 그건 이미 실측 검증
완료)다. 부하테스트는 비용이 드는 작업이라 배포 직전으로 미룸.
